...
def clean_json_response(content: str) -> str:
    """Clean JSON response by removing wrapping and whitespace."""
...
def create_chat_model(
    ...
    return ChatOpenAI(
        base_url=base_url, api_key=api_key, model=model_name, temperature=temperature
    )
...
def create_chat_prompt_from_files(
    system_message_file: str, question_message_file: str
) -> ChatPromptTemplate:
...
def parse_json_response(
    response_content: str, model_class: Optional[Type[BaseModel]] = None
) -> tuple[bool, Any]:
    """
...
def create_structured_prompt(question: str, response_schema: Dict[str, str]) -> str:
    ...
    schema_example = json.dumps(response_schema, indent=4, ensure_ascii=False)
...
# ...existing code from common.py will be copied here...

# add test for clean_json_response
if __name__ == "__main__":
    print(clean_json_response('```json\n{"key": "value"}\n```'))
